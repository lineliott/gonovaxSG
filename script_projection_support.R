...
#   idx <- stratum_index_xvwv(1, n_diag_rec)#eliott
#   
#   n_vax <- idx$n_vax
#   
#   i_v <- idx$V
...
#   
#   # set up uptake matrix rows = groups, columns = vaccine strata
#   # u <- create_uptake_map_xvwv(n_group = n_group, n_vax = n_vax,
...
################################################################
# run_grid_xvwr<- function (gono_params, init_params, cost_params, baseline, model, 
#                    dur, dur_revax,
#                    primary_uptake = 0, booster_uptake = 0,
#                    uptake_total = 0, uptake_second_dose = uptake_total,
...
#                   1)
# 
#   # run model (sut up for onevax_xvwr)
#   l <- expand.grid(vea = eff, vea_revax = eff,
#                    vei = eff, vei_revax = eff,
#                    ved = eff, ved_revax = eff,
#                    ves = eff, ves_revax = eff,
#                    dur = dur, dur_revax = dur_revax,
#                    primary_uptake = primary_uptake, booster_uptake = booster_uptake)
#   res <- furrr::future_pmap(.l = l, .f = model, tt = tt, gono_params = gono_params,
#                             init_params = init_params, vbe = vbe,
...
# 
# 
# uptake_control <- function(uptake, cost_eff_params) {
#   if (uptake == "central") {
#     uptake_total <- cost_eff_params$uptake_total
#   } else if (uptake == "low") {
#     uptake_total <- cost_eff_params$uptake_total / 2
#   } else if (uptake == "high") {
#     uptake_total <- cost_eff_params$uptake_total * 2
#   } else {
#     uptake_total <- uptake
#   }
# 
#   list(total = uptake_total,
#        second_dose = cost_eff_params$uptake_second_dose)
# }
